...
import torch_xla
import torch_xla.core.xla_model as xm # for using 1 TPU cores
...
class Trainer():
    ...
    def __init__(self, model, optimizer, loss, metrics, device, logger, amp, interval=100):
        ...
        self.device = device # in train.py device should be different
    ...
    def train(self, mode, dataloader, epoch_index=0):
        ...
        for batch_index, (x,y) in enumerate(tqdm(dataloader)):
            ...
            if mode == 'train':
                ...
                #self.optimizer.step()
                xm.optimizer_step(self.optimizer, barrier=True) # instead of self.optimizer.step() use this for tpu
